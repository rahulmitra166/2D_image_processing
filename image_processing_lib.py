...
from skimage.morphology import remove_small_objects, convex_hull_image, h_maxima
...
class ParticleAnalyzer:
    ...
    def remove_blur_and_detect_cores(self, image, se_radius=3, threshold=None):
        """
        Reduce blurring halo around bright particles and detect the sharp cores.

        Uses a morphological top-hat filter to subtract the halo, then thresholds to get a binary mask of cores.

        Parameters
        ----------
        raw_image : 2D or 3D numpy.ndarray
            Grayscale or BGR image containing bright particles on a dark background.
        se_radius : int, optional
            Radius (in pixels) of the elliptical structuring element for the top-hat filter.
        threshold : float or None, optional
            Manual threshold for binarization; if None, Otsu's method is used.

        Returns
        -------
        tophat : 2D numpy.ndarray
            Result of the top-hat filtering (highlighted cores).
        mask : 2D numpy.ndarray of uint8
            Binary mask where detected core pixels are 255.
        """

        # Create an elliptical structuring element
        ksize = se_radius #2 * se_radius + 1
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (ksize, ksize))

        # Apply top-hat filtering to remove the halo
        tophat = cv2.morphologyEx(image, cv2.MORPH_TOPHAT, kernel)

        # Threshold the result to get binary cores
    ...
    def calculate_particle_properties_contours(self, binary_image, contours, experiment, image, conversion_factor):
        """
    ...
    def watershed_segmentation(self, binary_image, min_area, max_area):
        """
        Perform watershed segmentation on a 2D binary image using OpenCV only.

        Parameters
        ----------
        binary_image : 2D numpy.ndarray of uint8 or bool
            Binary image where foreground pixels are non-zero and background is zero.
        min_area : int
            Minimum area of particles to be considered valid.
        max_area : int
            Maximum area of particles to be considered valid.

        Returns
        -------
        labels : 2D numpy.ndarray of int32
            Labeled image after watershed; each segment has a unique positive integer label.
        overlay : 3D numpy.ndarray of uint8
            BGR image overlay of color-coded labels on top of the original image.
        """
        # Create a circular mask
        mask = np.zeros(binary_image.shape, dtype=np.uint8)
        center = (binary_image.shape[0] // 2, binary_image.shape[1] // 2)
        rr, cc = draw.disk(center, radius=1250, shape=binary_image.shape)
        mask[rr, cc] = 1
        ...
        dist = cv2.distanceTransform(binary_image, distanceType=cv2.DIST_L2, maskSize=5)

        # Normalize distance image for display
        # cv2.normalize(dist, dist, 0, 1.0, cv2.NORM_MINMAX)

        # Threshold to obtain markers
        _, sure_fg = cv2.threshold(dist, 3, 255, cv2.THRESH_BINARY)
        ...
        sure_bg = cv2.dilate(binary_image, np.ones((7,7), np.uint8), iterations=3)
        unknown = cv2.subtract(sure_bg, sure_fg)
        markers[unknown == 255] = 0

        # Convert original binary to BGR for watershed input
        ...
        for region in measure.regionprops(labels):
            # Check if the region touches the image boundary or the circular mask boundary
            min_row, min_col, max_row, max_col = region.bbox
            if (min_row == 0 or min_col == 0 or max_row == binary_image.shape[0] or max_col == binary_image.shape[1] or
                not np.all(mask[region.coords[:, 0], region.coords[:, 1]] == 1)):
                continue
            
            if min_area <= region.area <= max_area:
                filtered_labels[labels == region.label] = region.label

        overlay = label2rgb(filtered_labels, image=binary_image, bg_label=0, alpha=1)
        return overlay, filtered_labels

    
    def calculate_particle_properties_watershed(self, labels, experiment, image, min_area, conversion_factor):
        ...
        if len(regions) > 0:  # Only process if regions exist
            for region in regions:
                if region.area > min_area:
                    area = region.area
                    # Create a binary mask of the particle
                    binary_mask = (labels == region.label).astype(np.uint8)
                    
                    # Ensure binary_mask contains valid data
                    if np.any(binary_mask):  # Check if the mask contains any `True` values
                        # Compute perimeter of the particle
                        perimeter = measure.perimeter(binary_mask, neighborhood=8)
                        
                        # Compute convex hull of the particle
                        convex_hull = convex_hull_image(binary_mask)
                        convex_hull_perimeter = measure.perimeter(convex_hull, neighborhood=8)
                        
                        # Calculate convexity (convex hull perimeter / particle perimeter)
                        convexity = max(convex_hull_perimeter / perimeter, 1) if perimeter > 0 else 1
                        
                        # Calculate max and min Feret diameter
                        max_feret = feret.max(binary_mask)
                        min_feret = feret.min(binary_mask)
                        
                        # Calculate other properties
                        equivalent_diameter = region.equivalent_diameter
                        axis_major = region.major_axis_length
                        axis_minor = region.minor_axis_length
                        solidity = region.solidity
                        aspect_ratio = min_feret / max_feret if max_feret > 0 else 0
                        ...
                        props = {
                            ...
                            'max_feret': max_feret * conversion_factor,
                            'min_feret': min_feret * conversion_factor,
                            'axis_major': axis_major * conversion_factor,
                            'axis_minor': axis_minor * conversion_factor,
                            'solidity': solidity,
                        ...
                        if properties.empty:
                            ...
                            properties = pd.concat([properties, props_df], ignore_index=True)
        return properties if not properties.empty else None
